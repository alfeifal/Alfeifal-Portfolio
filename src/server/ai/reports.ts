...
export async function dailyReview(user: SessionUser, date?: string) {
  ...
  const data = { date: day, doneTasks: doneTasks.map((t) => t.title), openOrOverdue: openToday.map((t) => ({ title: t.title, due: t.dueDate, priority: t.priority })), events: events.map((e) => ({ title: e.title, kind: e.kind })), training: { plannedDay: workout?.day?.name ?? null, isRest: workout?.day?.isRest ?? null, sessions: sessions.map((s) => ({ day: s.dayName, sets: s.sets, volume: s.volume, finished: !!s.finishedAt })) }, finance: { expenses: finance.expenses, income: finance.income, top: finance.byCategory.slice(0, 5) }, study: { minutes: study.totalMinutes, bySubject: study.bySubject }, german: { minutes: german.totalMinutes, streak: german.streak }, nutrition: { totals: nutrition.totals, goals: nutrition.goals, estimatedItems: nutrition.estimatedItems }, journal: journal.map((j) => ({ kind: j.kind, mood: j.mood, text: j.content.slice(0, 300) })) };
