...
defineTool({
  ...
  run: async (i, ctx) => { void ctx; await m.refreshNews(i.refresh).catch(() => 0); return (await m.listNews(i)).map((n) => ({ id: n.id, headline: n.headline, source: n.source, publishedAt: n.publishedAt, url: n.url, category: n.category, summary: n.summary })); },
});
defineTool({
