...
defineTool({ name: "create_goal", module: "goals", risk: "low", description: `Create a goal: an outcome to reach, measured by a metric. Use create_project for a body of work with tasks. Optional metric (name/unit/target) makes progress computable. ${LINKED}`, schema: g.goalCreateSchema.omit({ source: true }), summarize: (i) => `create_goal — ${i.name}`, run: (i, ctx) => g.createGoal(ctx.user.id, { ...i, source: "ai" }, ctx.user.timezone, "ai") });
...
defineTool({ name: "add_milestone", module: "goals", risk: "low", description: "Add a milestone to a GOAL (add_project_milestone is the project equivalent). Tick it with complete_milestone.", schema: z.object({ goalId: z.string().uuid(), title: z.string(), dueDate: z.string().optional() }), summarize: (i) => `add_milestone — ${i.title}`, run: (i, ctx) => g.addMilestone(ctx.user.id, i.goalId, { title: i.title, dueDate: i.dueDate, position: 0 }, "ai") });
