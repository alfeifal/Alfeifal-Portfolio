...
export const GET = withAuth(async (_req, { user }) => json({ id: user.id, email: user.email, name: user.name, role: user.role, isActive: user.isActive, mustChangePassword: user.mustChangePassword, createdAt: user.createdAt, lastLoginAt: user.lastLoginAt, timezone: user.timezone, currency: user.currency, locale: user.locale, preferences: await getPreferences(user.id), aiConfigured: aiConfigured(), marketProviders: { finnhub: Boolean(process.env.FINNHUB_API_KEY), stooq: true, yahoo: true, coingecko: true, newsRss: true } }));
