...
interface Day { date: string; meals: { id: string; type: string; name: string | null; loggedAt: string; source: string; items: Entry[] }[]; totals: { calories: number; protein: number; carbs: number; fat: number }; consistency: Consistency & { entriesOff: number }; goals: { calories: number; protein: number; carbs: number; fat: number }; estimatedItems: number; exactItems: number }
        ...
        <>
          ...
          {d.meals.length === 0 ? <Empty title="No meals logged for this day" action={<><Button variant="primary" size="sm" onClick={() => setModal("meal")}>Log a meal</Button>{aiConfigured && <Button size="sm" onClick={() => setModal("quick")}>Describe it to the AI</Button>}</>}>Meals and macros for {fmtDate(date)} will appear here.</Empty> : d.meals.map((m) => (
            <Card key={m.id} title={<span className="capitalize">{m.type.replace("_", " ")}{m.name ? ` · ${m.name}` : ""}</span>} action={<button className="btn-ghost btn-sm" onClick={() => confirm(async () => { await api(`/api/nutrition/meals/${m.id}`, { method: "DELETE" }); toast.success("Meal deleted"); all(); }, { title: "Delete meal?" })}>✕</button>}>
              <ul className="divide-y divide-border text-sm">{m.items.map((i) => <li key={i.id} className="flex items-center gap-2 py-1"><span className="flex-1">{i.description} <span className="muted">× {i.quantity} {i.unit}</span> <Source source={i.source} />{i.confidence != null && <span className="text-[10px] muted"> {i.confidence}%</span>}{!i.consistency.ok && <Badge tone="warning" className="ml-1">macros ≈ {Math.round(i.consistency.macroCalories)} kcal</Badge>}</span><span className="tnum text-xs muted">{fmtNum(i.calories, 0)} kcal · P {fmtNum(i.protein, 0)} · C {fmtNum(i.carbs, 0)} · F {fmtNum(i.fat, 0)}</span><button className="btn-ghost btn-sm" onClick={async () => { await api(`/api/nutrition/entries/${i.id}`, { method: "DELETE" }); all(); }}>✕</button></li>)}</ul>
            </Card>
          ))}
