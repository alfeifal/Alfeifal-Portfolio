...
import { Badge, Card, Empty, ErrorBox, Field, Modal, PageHeader, Spinner, Stat, Tabs, Markdown, Button, SkeletonStats, useConfirm, usePrompt } from "@/components/ui";
...
export default function TradingPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (modal === "account") {
        // Editing keeps the mode: an account cannot cross the simulated/real line with its trades.
        const body = { name: form.name, broker: form.broker || null, startingBalance: Number(form.startingBalance || 0), riskPerTradePct: Number(form.riskPerTradePct || 1) };
        if (editingAccount) await api(`/api/trading/accounts/${editingAccount.id}`, { method: "PATCH", json: body });
        else await api("/api/trading/accounts", { method: "POST", json: { ...body, mode: form.mode || "paper" } });
      }
      ...
      else if (modal === "account") toast.success(editingAccount ? "Account updated" : "Account created", form.name);
      else toast.success(modal === "trade" ? "Trade logged" : modal === "watch" ? "Added to watchlist" : modal === "alert" ? "Alert created" : "Event added", modal === "trade" ? `${form.symbol} ${form.direction ?? "long"} · ${mode === "real" ? "REAL" : "simulated"}` : form.symbol ?? form.name ?? form.title);
      setModal(null); setForm({}); setEditingStrategy(null); setEditingAccount(null); all();
    ...
    <div className="space-y-4">
                  ...
                  return (
                    <li key={st.id} className="flex items-start gap-3 py-2">
                      ...
                      <span className="flex shrink-0 gap-1.5">
                        ...
                        <Button size="sm" variant="danger" onClick={() => confirm(async () => { await api(`/api/trading/strategies/${st.id}`, { method: "DELETE" }); strategies.refresh(); toast.success("Strategy archived", st.name); }, { title: "Archive this strategy?", description: `${st.name} stops being offered. Trades already tagged with it keep naming it and your statistics do not change.`, confirmLabel: "Archive" })}>Archive</Button>
                      </span>
                    </li>
                  );
                })}
      ...
      {tab === "brief" && (
        <Card title="Daily Market Brief" action={<button className="btn-primary btn-sm" disabled={busy || !aiConfigured} onClick={genBrief}>{busy ? "Generating…" : "Generate today's brief"}</button>}>
          ...
          {brief.data ? <><p className="mb-2 text-xs muted">Generated {fmtDate(brief.data.createdAt, { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</p><Markdown text={brief.data.content} /></> : <p className="text-sm muted">No brief yet. Generate one to get today&apos;s headlines, your watchlist quotes and your registered events in one page.</p>}
        </Card>
      )}
      <Modal open={modal !== null} onClose={() => { setModal(null); setEditingStrategy(null); setEditingAccount(null); }} title={modal === "trade" ? `New ${mode} trade` : modal === "watch" ? "Add to watchlist" : modal === "alert" ? "Price alert" : modal === "account" ? (editingAccount ? "Edit trading account" : "New trading account") : modal === "strategy" ? (editingStrategy ? "Edit strategy" : "New strategy") : "Economic event"} wide={modal === "trade" || modal === "strategy"}>
...
function WatchlistView({ watchlists, quotes, err, onRemove, onNote }: { watchlists: Watchlist[]; quotes: { quotes: Quote[]; missing: string[] } | null; err: string; onRemove?: (id: string, symbol: string) => void; onNote?: (id: string, notes: string, symbol: string) => void }) {
  ...
  return (
    <div>
      ...
      <ul className="divide-y divide-border text-sm">{items.map((i) => { const q = quotes?.quotes.find((x) => x.symbol === i.symbol); return <li key={i.id} className="flex items-center gap-2 py-1.5"><span className="flex-1"><span className="font-medium">{i.symbol}</span> <span className="text-xs muted">{i.assetClass}</span>{i.notes && <span className="block text-xs muted">{i.notes}</span>}</span>{q ? <span className="tnum text-right"><span>{fmtNum(q.price, 2)}</span> <span className={(q.changePct ?? 0) >= 0 ? "text-positive" : "text-negative"}>{q.changePct != null ? `${q.changePct >= 0 ? "+" : ""}${fmtNum(q.changePct, 2)}%` : ""}</span><span className="block text-[10px] muted">{q.provider} · {q.freshness}</span></span> : quotes?.missing.includes(i.symbol) ? <span className="text-xs muted">no price</span> : null}{onNote && <button className="btn-ghost btn-sm" onClick={() => onNote(i.id, i.notes ?? "", i.symbol)}>note</button>}{onRemove && <button className="btn-ghost btn-sm" aria-label={`Remove ${i.symbol} from the watchlist`} onClick={() => onRemove(i.id, i.symbol)}>✕</button>}</li>; })}</ul>
    </div>
  );
}
