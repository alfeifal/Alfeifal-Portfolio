...
import { Badge, Card, ErrorBox, PageHeader, Spinner, Source } from "@/components/ui";
...
export default function SessionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-4">
      <PageHeader back={{ href: "/training", label: "Training" }} title={`${fmtDate(d.date, { weekday: "long", day: "numeric", month: "short" })} · ${d.dayName ?? "Session"}`} subtitle={`${d.sets.filter((x) => !x.isWarmup).length} sets · volume ${fmtNum(d.volume, 0)} kg${d.durationMinutes ? ` · ${d.durationMinutes} min` : ""}${d.finishedAt ? "" : " · still open"}`} action={<><button className="btn-ghost btn-sm" onClick={async () => { const v = prompt("Notes", d.notes ?? ""); if (v == null) return; await api(`/api/training/sessions/${id}`, { method: "PATCH", json: { notes: v } }); s.refresh(); }}>Notes</button><button className="btn-ghost btn-sm text-negative" onClick={async () => { if (confirm("Delete this session and its sets?")) { await api(`/api/training/sessions/${id}`, { method: "DELETE" }); router.push("/training"); } }}>Delete</button></>} />
      ...
      {d.sets.length === 0 && <p className="text-sm muted">No sets logged in this session.</p>}
    </div>
  );
}
