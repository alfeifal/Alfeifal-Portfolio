...
interface HistoryItem { id: string; date: string; dayName: string | null; sets: number; volume: number; finishedAt: string | null; durationMinutes: number | null; status: "started" | "in_progress" | "completed" | "empty"; isWorkout: boolean }
      ...
      {d && (
        <>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <Card title="History" action={<Link href="/training/records" className="btn-ghost btn-sm">Records</Link>}>{!history.data?.length ? <p className="text-sm muted">No workouts yet — start today&apos;s session and log your first sets.</p> : <ul className="divide-y divide-border text-sm">{history.data.map((s) => <li key={s.id}><Link href={`/training/sessions/${s.id}`} className="flex justify-between py-1.5 hover:underline"><span>{fmtDate(s.date)} · {s.dayName ?? "Free session"}{!s.finishedAt && <Badge tone="warning" className="ml-1">open</Badge>}{s.status === "empty" && <Badge className="ml-1">no sets</Badge>}</span><span className="muted tnum">{s.sets} sets · {fmtNum(s.volume, 0)} kg{s.durationMinutes ? ` · ${s.durationMinutes} min` : ""}</span></Link></li>)}</ul>}</Card>
          </div>
        </>
      )}
