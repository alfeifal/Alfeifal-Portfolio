...
/**
 * Home's interactive tree.
 *
 * It stays a Client Component because the whole page is animated (Stagger/motion), the task list
 * completes optimistically, and the ask box and quick-entry button need handlers and shell context.
 * What changed is where its first data comes from: the Server Component above loads it and passes it
 * in, so this renders real content on the first paint and never fetches on mount. `useApi` keeps the
 * path, so a refresh or a module invalidation after a mutation still re-reads from the server.
 */
export default function HomeClient({ initial }: { initial: DashboardData }) {
  const { user, aiConfigured, openQuick } = useShell();
  const toast = useToast();
  const { data, error, loading, reload, refresh, setData } = useApi<DashboardData>("/api/dashboard", [], initial);
